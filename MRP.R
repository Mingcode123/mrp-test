...
for (b in reps) {
  ...
  mrp2_fit = stan_glmer(
    y ~ (1|age_group)+(1|sex)+(1|race_eth)+(1|state)+(1|state_race),
    family = binomial(link = "logit"),
    data = hps_sample,
    weights = model_w,
    chains = chains,
...
for (b in reps) {
  ...
  if (!(b %in% hps_sample_ids$replication)) {
    cat("Skipping MRP4 replication", b, ": sample ids not found\n")
    next
  }
  
  if (file.exists("results/mrp4/estimates.rds")) {
    old_mrp4_estimates = read_rds("results/mrp4/estimates.rds")
    
    if (b %in% old_mrp4_estimates$replication) {
      cat("Skipping MRP4 replication", b, ": already in estimates\n")
      next
    }
  }
  
  cat("MRP4 replication", b, "\n")
  ...
  mrp4_fit = stan_glmer(
    y ~ (1|age_group)+(1|sex)+(1|race_eth)+(1|state)+(1|age_race)+(1|sex_race)+(1|age_sex),
    family = binomial(link = "logit"),
    data = hps_sample,
    weights = model_w,
    chains = chains,
  ...
  mrp4_state_estimates = tibble()
  
  for (s in states) {
    idx = poststrat_cells$state==s
    N_state = poststrat_cells$N_target[idx]
    theta_draw = as.numeric(mrp4_draws[,idx,drop = FALSE] %*% (N_state/sum(N_state)))
    
    mrp4_state_estimates = bind_rows(
      mrp4_state_estimates,
      tibble(
        state = factor(s, levels = states),
        theta_hat = mean(theta_draw),
        lower = quantile(theta_draw,0.025),
  ...
  mrp4_estimates_b = mrp4_state_estimates %>%
    mutate(
      method = "MRP4",
      replication = b
    ) %>%
  ...
  saveRDS(mrp4_fit, paste0("results/mrp4/fit_rep", b, ".rds"))
  
  if (file.exists("results/mrp4/estimates.rds")) {
    old_mrp4_estimates = read_rds("results/mrp4/estimates.rds")
    
    mrp4_estimates = old_mrp4_estimates %>%
      filter(replication!=b) %>%
      bind_rows(mrp4_estimates_b) %>%
      arrange(replication, state)
  } else {
    mrp4_estimates = mrp4_estimates_b
  }
  
  write_rds(mrp4_estimates, "results/mrp4/estimates.rds")
  write_csv(mrp4_estimates, "results/mrp4/estimates.csv")
}
...
for (b in reps) {
  ...
  if (!(b %in% hps_sample_ids$replication)) {
    cat("Skipping MRP5 replication", b, ": sample ids not found\n")
    next
  }
  
  if (file.exists("results/mrp5/estimates.rds")) {
    old_mrp5_estimates = read_rds("results/mrp5/estimates.rds")
    
    if (b %in% old_mrp5_estimates$replication) {
      cat("Skipping MRP5 replication", b, ": already in estimates\n")
      next
    }
  }
  
  cat("MRP5 replication", b, "\n")
  ...
  mrp5_fit = stan_glmer(
    y ~ (1|age_group)+(1|sex)+(1|race_eth)+(1|state)+(1|age_race)+(1|sex_race)+(1|age_sex)+(1|state_race),
    family = binomial(link = "logit"),
    data = hps_sample,
    weights = model_w,
    chains = chains,
  ...
  mrp5_state_estimates = tibble()
  
  for (s in states) {
    idx = poststrat_cells$state==s
    N_state = poststrat_cells$N_target[idx]
    theta_draw = as.numeric(mrp5_draws[,idx,drop = FALSE] %*% (N_state/sum(N_state)))
    
    mrp5_state_estimates = bind_rows(
      mrp5_state_estimates,
      tibble(
        state = factor(s, levels = states),
        theta_hat = mean(theta_draw),
        lower = quantile(theta_draw,0.025),
  ...
  mrp5_estimates_b = mrp5_state_estimates %>%
    mutate(
      method = "MRP5",
      replication = b
    ) %>%
  ...
  saveRDS(mrp5_fit, paste0("results/mrp5/fit_rep", b, ".rds"))
  
  if (file.exists("results/mrp5/estimates.rds")) {
    old_mrp5_estimates = read_rds("results/mrp5/estimates.rds")
    
    mrp5_estimates = old_mrp5_estimates %>%
      filter(replication!=b) %>%
      bind_rows(mrp5_estimates_b) %>%
      arrange(replication, state)
  } else {
    mrp5_estimates = mrp5_estimates_b
  }
  
  write_rds(mrp5_estimates, "results/mrp5/estimates.rds")
  write_csv(mrp5_estimates, "results/mrp5/estimates.csv")
}